...
def ValidityCheck(y,SpMatrix):
    ...
    Smatrix=SpMatrix.toarray()    
    checkY=y.T.dot(Smatrix)
...
def yRoundFilter(y,n):
    ...
    for i in range(len(y)):
        ...
        y[i]=int((10**(order))*y[i])
        
    uy=np.unique(y).astype(np.uint)
    GCD=np.gcd.reduce(list(uy))
...
def Inequality(Graph,inflation_order,card,Sol,b,SpMatrix):
    ...
    if Comp == False:
        ...
        obs_count,num_vars,exp_set,group_elem,det_assumptions,names=inflation.LearnInflationGraphParameters(Graph,inflation_order)
        ...
        return print('Compatibility Error: The input distribution is compatible with the inflation graph ')
