...
DataInstrumental = np.zeros(8);
DataInstrumental[0] = 0.5;
DataInstrumental[5] = 0.5;
...
def Inequality(Graph, inflation_order, card, Sol):
    ...
    obs_count, num_vars, exp_set, group_elem, det_assumptions, names = inflation.LearnInflationGraphParameters(
        InstrumentalGraph, inflation_order)
    ...
    ListingProbs2 = product(ListingProbs1, repeat=inflation_order)
    ListingProbs3 = ['x'.join(v) for v in ListingProbs2]

    ListingProbs4 = []
    for i in np.unique(G):

        indecies = np.where(G == i)[0]
        string = ListingProbs3[indecies[0]]
...
I = Inequality(Graph, inflation_order, card, Sol)
