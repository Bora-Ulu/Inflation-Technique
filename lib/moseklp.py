...
def InfeasibilityCertificateAUTO(A,b):
    with mosek.Env() as env:
        env.set_Stream(mosek.streamtype.log, streamprinter)
        with env.Task(0, 0) as task:
            task.set_Stream(mosek.streamtype.log, streamprinter)
            ...
            task.putobjsense(mosek.objsense.minimize)
            task.putintparam(mosek.iparam.bi_clean_optimizer, mosek.optimizertype.dual_simplex)
            task.putintparam(mosek.iparam.presolve_use, mosek.presolvemode.off)
            task.putintparam(mosek.iparam.optimizer, mosek.optimizertype.intpnt)
...
if __name__ == '__main__':
    ...
    Sol = InfeasibilityCertificateAUTO(A, b+1/len(b))
    Sol['x']
